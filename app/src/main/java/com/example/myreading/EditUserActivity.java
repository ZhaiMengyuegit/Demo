...
public class EditUserActivity extends BaseActivity {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.resetpwd_btn_cancel:                     //取消按钮的监听事件,由注册界面返回登录界面
                    Intent intent_Resetpwd_to_Login = new Intent(EditUserActivity.this,LoginActivity.class) ;    //切换Resetpwd Activity至Login Activity
            ...
            if(result==1){                                             //返回1说明用户名和密码均正确,继续后续操作
                if(userPwd_new.equals(userPwdCheck)==false){           //两次密码输入不一样
                    ...
                    if (flag == false) {
                        ...
                        Intent intent_Register_to_Login = new Intent(EditUserActivity.this,LoginActivity.class) ;    //切换User Activity至Login Activity
    ...
    public boolean isUserNameAndPwdValid() {
        ...
        if (mAccount.getText().toString().trim().equals("")) {
            ...
            Toast.makeText(this, "新密码核实不能为空",Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
